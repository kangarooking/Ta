...
### 2026-08-31 09:56 — 新增「为什么做 Windows 版」章节

- 新增：README 补充 Windows 版开发背景——解决 Codex CLI 无法直接粘贴图片的痛点，截图即得文件路径、结果页支持复制图片与 Esc 快速关闭，取字默认使用智谱免费 `glm-4v-flash` 模型。
...
Windows 版的诞生，源于 CLI 里一个每天都要面对的真实痛点。

在使用 Codex CLI 这类命令行 AI 工具时，经常需要把屏幕上的内容交给它分析：一段报错、一张界面图、一份文档截图。但 CLI 不能直接粘贴图片，当时的办法只能是用其他截图工具把图片先存到桌面，再找到文件、复制路径、回到 CLI 粘贴——一张截图要绕四步，思路经常被打断。

所以这个 Windows 版最重要的设计只有一条：**截图完成的瞬间，文件路径就已经在剪贴板里了**。截完图回到 CLI 直接 `Ctrl+V`，路径就进去了，一次都不用来回切换。

- **截图即得路径**：截图自动保存到本机临时目录，完整文件路径在剪贴板排他锁内安全写入，回到 CLI 直接粘贴；
- **需要图片本体也可以**：结果页面提供「复制图片」按钮，一键把 PNG 放进剪贴板，粘贴到微信、文档或任何支持图片的地方；
- **免费模型取字**：文字识别默认接入智谱免费开放的 `glm-4v-flash` 多模态模型，识别结果自动复制并保存 `.vision.txt`，日常取字零成本；
- **看完就走**：结果窗口按 `Esc` 立即关闭，不必移动鼠标去找关闭按钮。

Windows 版目前处于 Alpha 阶段，源码与构建说明见 [`windows/`](./windows/README.md)。
