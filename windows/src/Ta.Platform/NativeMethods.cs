...
internal static class Native
{
    ...
    public const uint WS_EX_LAYERED = 0x0008_0000;     // 分层窗口 —— SetLayeredWindowAttributes 的前提
    ...
    // 自定义消息：把钩子线程上的 Esc 切回窗口线程收尾。
    public const uint WM_APP = 0x8000;
    public const uint WM_APP_CANCEL = WM_APP + 1;
    public const uint WM_APP_ENTER = WM_APP + 2;

    // RegisterClassEx 的"类已存在"错误码 —— 复用静态类名时属预期。
    ...
    // GetModuleHandle 属于 kernel32，不是 user32 —— 放错库会得到
    // EntryPointNotFoundException，且只在真正创建窗口时才暴露。
    ...
    // ── 合成按键 ──────────────────────────────────────────────────
    // 仅供自动回归验证钩子链路：低级钩子同样能观察到合成输入。
    public const uint KEYEVENTF_KEYUP = 0x0002;

    [DllImport(User32, EntryPoint = "keybd_event")]
    public static extern void keybd_event(byte bVk, byte bScan, uint dwFlags, IntPtr dwExtraInfo);

    /// <summary>合成一次按键按下与松开。</summary>
    public static void SendSyntheticKey(byte virtualKey)
    {
    ...
    /// <summary>BLACK_BRUSH 库存画刷（GetStockObject 的索引常量）。</summary>
